...
get_latest_release() {
    ...
    if command -v curl &>/dev/null; then
        ...
        if [[ "$http_status" == "200" ]]; then
            ...
            warning "GitHub API returned HTTP $http_status" >&2
            release_data=""
        fi
    ...
    if [[ -z "$release_data" ]] || [[ "$release_data" == *"Not Found"* ]]; then
        warning "Cannot fetch from GitHub, using bundled version: $CURRENT_VERSION" >&2
        return
    fi
    ...
    success "Latest version: $CURRENT_VERSION" >&2
    echo "$zip_url"
}
...
download_source() {
    ...
    if command -v curl &>/dev/null; then
        curl -sL -w "\nHTTP_CODE:%{http_code}" "$url" -o "${temp_dir}/source.zip" 2>/dev/null
        download_status=$?
    ...
    if [[ -d "$extracted" ]]; then
        ...
        warning "No extracted directory found, using bundled files"
    fi
