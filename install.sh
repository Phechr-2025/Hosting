...
get_latest_release() {
    ...
    if command -v curl &>/dev/null; then
        http_status=$(curl -sL -o /dev/null -w "%{http_code}" "$api_url" 2>/dev/null)
        if [[ "$http_status" == "200" ]]; then
            release_data=$(curl -sL "$api_url" 2>/dev/null)
        else
            warning "GitHub API returned HTTP $http_status"
            release_data=""
        fi
...
download_source() {
    ...
    if command -v curl &>/dev/null; then
        curl -sL "$url" -o "${temp_dir}/source.zip" 2>/dev/null
        download_status=$?
        if [[ $download_status -ne 0 ]]; then
            error "curl download failed (exit code: $download_status)"
            rm -rf "$temp_dir"
            return 1
        fi
    elif command -v wget &>/dev/null; then
        wget -q "$url" -O "${temp_dir}/source.zip" 2>/dev/null
        download_status=$?
        if [[ $download_status -ne 0 ]]; then
            error "wget download failed (exit code: $download_status)"
            rm -rf "$temp_dir"
            return 1
        fi
    fi
    ...
    local file_size=$(stat -c%s "${temp_dir}/source.zip" 2>/dev/null || echo "0")
    if [[ "$file_size" -lt 1000 ]]; then
        error "Downloaded file is too small ($file_size bytes) - likely a 404 error page"
        rm -rf "$temp_dir"
        return 1
    fi
    ...
    if ! unzip -q "${temp_dir}/source.zip" -d "$temp_dir" 2>/dev/null; then
    ...
    if [[ -d "$extracted" ]]; then
        ...
        success "Source code extracted and copied"
    else
        warning "No extracted directory found"
    fi
...
APACHEEOF
            ...
            if [[ "$SEC_HTTPS" == "true" && "$SSL_PROVIDER" != "http" ]]; then
                cat >> "$APACHE_CONF" <<'APACHEEOF'
    RewriteEngine On
...
APACHEEOF
            ...
            if [[ "$SEC_HSTS" == "true" ]]; then
                echo "    Header always set Strict-Transport-Security 'max-age=31536000; includeSubDomains'" >> "$APACHE_CONF"
            fi
            if [[ "$SEC_FRAME" == "true" ]]; then
                echo "    Header always set X-Frame-Options 'SAMEORIGIN'" >> "$APACHE_CONF"
            fi
            if [[ "$SEC_XSS" == "true" ]]; then
                echo "    Header always set X-XSS-Protection '1; mode=block'" >> "$APACHE_CONF"
            fi
            if [[ "$SEC_CSP" == "true" ]]; then
                echo "    Header always set Content-Security-Policy 'default-src self;'" >> "$APACHE_CONF"
            fi
...
install_hosting_command() {
    ...
    # ถ้า hosting.sh ไม่อยู่ใน INSTALL_DIR (download ล้มเหลว) ให้ดาวน์โหลดจาก GitHub raw
    if [[ ! -f "$INSTALL_DIR/hosting.sh" ]]; then
        log "hosting.sh not found in install dir, downloading from GitHub..."
        if command -v curl &>/dev/null; then
            curl -sL "https://raw.githubusercontent.com/${GITHUB_REPO}/main/hosting.sh" -o "$INSTALL_DIR/hosting.sh" 2>/dev/null || true
        elif command -v wget &>/dev/null; then
            wget -q "https://raw.githubusercontent.com/${GITHUB_REPO}/main/hosting.sh" -O "$INSTALL_DIR/hosting.sh" 2>/dev/null || true
        fi
    fi
    ...
    if [[ -f "$INSTALL_DIR/hosting.sh" ]]; then
        ...
        error "hosting.sh not found - menu command will not be available"
        error "Please download hosting.sh manually from: https://github.com/${GITHUB_REPO}"
    fi
...
main() {
    ...
    if [[ -n "$url" ]]; then
        if ! download_source "$url"; then
            warning "Download from GitHub failed, continuing with bundled installation..."
        fi
    else
        info "No remote release found, using bundled installation"
    fi
